...
|---|---|---|
| **Docker image** | ✅ Built in the cloud and published to `ghcr.io/arham-apon/gridwise:preli-1.0.0`, public and anonymously pullable. Verified `linux/amd64`, non-root `appuser`, port 8000, healthcheck, no secret in the image | Nothing |
| **Image digest in README** | ✅ `sha256:3bb1ba2467…56303`, in section 9 | Nothing |
| **Public endpoint** | ✅ Hosted on Render at `https://campus-light.onrender.com`, running the GHCR image. Independent of any laptop. 10/10 cases and 18/18 notes verified against it, p95 2.97 s | Keep a monitor pinging `/health` every 10 minutes — a free Render service sleeps after 15 minutes idle and takes ~1 minute to wake, which would exceed the 30 s limit |
| **3-minute video** | Not done | Tie-break only, no base points, but still on the checklist |
